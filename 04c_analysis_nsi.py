...
Everything is aggregated to WEEKS (~100-week window), which gives real statistical
power and lets us see lead-lag (does anxiety follow alarms with a delay?).

Alarms are weighted by how WIDESPREAD the threat was over the WEEK: the weekly
alarm count is multiplied by (distinct regions under alert that week / 7).
...
# Each alarm is weighted by how widespread the threat was that day:
# breadth = (distinct regions under alert that day) / 7  -> a countrywide day
# counts more than a pinpoint local one.
...
def main():
    ...
    # Weight each WEEK by how widespread the threat was across the whole week:
    # weekly breadth = (distinct regions under alert during the week) / 7, multiplied
    # by the week's total alarm count. Needs the 'region' column from 03; if it is
    # missing (raw fallback) every alarm just counts as 1.
    if "region" in a.columns:
        regions_per_week = a[a["region"].isin(REAL_REGIONS)].groupby("week")["region"].nunique()
        breadth_week = (regions_per_week / len(REAL_REGIONS))
        alarms = (a.groupby("week").size() * breadth_week).rename("alarms")
    else:
    ...
    print("\n=== lead-lag: anxiety_t vs NSI_(t-k weeks) ===")
    for k in range(0, 4):
        r, p, n = corrp(d[X], d[M].shift(k))
        print(f"  NSI lead by {k} wk: r={r:5.2f} p={p:.4f} {stars(p)}")
