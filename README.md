...
predictors alarms are the stronger one. The result is robust to the region
threshold (≥3 or ≥4 of 7 give the same conclusion). The alarm→anxiety link is strongest at a
...
| `04c_analysis_nsi.py` | **Main analysis (weekly, 100-wk window).** Alarms are counted only on days when **≥4 of the 7 regions** were under alert (widespread threat, not pinpoint); weeks with no widespread threat count as 0. Media coverage = NSI (sentiment × trauma-topic density). Correlations + lead-lag + regression. Alarms predict anxiety more strongly, strongest at a ~1-week lag. **Writes `weekly_nsi_analysis.csv`** (used by 05 and 06). |
...
## Notes / decisions
- Resolution is weekly (100-week window, Apr 2023–Feb 2025). Alarms are the
  count of widespread-threat alarms (days hitting ≥4 of 7 regions). The lead-lag
  check shows anxiety follows alarms by about one week.
