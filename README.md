...
| `04c_analysis_nsi.py` | **Main analysis (weekly, ~84 wks).** Media coverage = NSI (sentiment × trauma-topic density). Compares alarms vs coverage as predictors of anxiety, including lead-lag. Alarms predict anxiety more strongly, strongest at a ~1-week lag. **Writes `weekly_nsi_analysis.csv`** (the merged weekly table used by 05 and 06). |
